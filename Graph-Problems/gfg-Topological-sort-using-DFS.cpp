...
class Solution
{
	...
	void dfs(int source,vector<bool>& visited,vector<int> adj[])
	{
	    ...
	    /*
	        Since we are adding to the element after the recursive call is finished and the source vertex is 
	        responcible for addition of all the childred/ adj vertexes so we make sure to add it at the top
	        of the stack of the topological sort 
	        
	        Which will eventually happen with the following setting of code the parent vertext will alwayd be
	...
	vector<int> topoSort(int V, vector<int> adj[]) 
	{
	    // code here
	    /*
	        Normal method of calling the dfs functions
